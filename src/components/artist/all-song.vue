...
import { type SongEntry } from '@/api/netease/searchSongs';
...
const fetchSongs = async (page: number, key: string) => {
  ...
  const result: any = (await artist.getArtistAllSongs(key, 'hot', 15, (page - 1) * 15)).data;
  ...
  const pics = await getSongApi.getSongPics(songList.map((s) => s.id));
  for (let i = 0; i < songList.length; i++) {
    songList[i].picUrl = pics[i];
  }

  songCount.value = result.total;
